...
class ThreeTenPrim implements ThreeTenAlg {
    /**
     * Wrapper class for nodes to add additional properties associated with this
     * algorithm.
     */
    private class NodeForAlg implements Comparable<NodeForAlg> {
         ...
         */
        public NodeForAlg(GraphNode node) {
            this.graphNode = node;
        }
    ...
    /**
     * Gets the next minimum id node from the available nodes. This is chosen as the
     * starting spot for the shortest path algorithm.
     * 
    ...
    public GraphNode getMinimumIdNode() {
        ...
        for (GraphNode a : graph.getVertices()) {
            minIdNode = a;
            break;
        }
    ...
    /**
     * Returns all the unused edges in the graph (for recoloring at the end).
     * 
    ...
    /**
     * Sets the node next to a given cost (or infinity if appropriate) while also
     * displaying the change from a previous value.
     * 
     * @param n       the node to set
