...
/**
 * This class is to create a graph class with a double array graph.
 * This class has some methods to make sure the place of the vertices,
 * edges and realize connecting, removing and other practices.
...
class ThreeTenGraph implements Graph<GraphNode, GraphEdge>, UndirectedGraph<GraphNode, GraphEdge> {
    ...
    /**
     * the default range of the double array is this number.
     */
    ...
    // no matter how well it works
    /**
     * the local variable that a double array.
     */
    private GraphComp[][] storage = null;

    // this is the only allowed constructor
    /**
     * the default constructor to create a double array.
     */
    ...
    public Collection<GraphEdge> getEdges() {
        ...
        Collection<GraphEdge> myEdge = new ArrayList<GraphEdge>();

        for (int i = 0; i < MAX_NUMBER_OF_NODES; i++) {
            for (int j = 0; j < MAX_NUMBER_OF_NODES; j++) {

                if (j > i && storage[i][j] != null) {
                    myEdge.add((GraphEdge) storage[i][j]);
                }
            }
        }
        return myEdge;
    ...
    public Collection<GraphNode> getVertices() {
        ...
        Collection<GraphNode> myVertice = new ArrayList<GraphNode>();

        for (int i = 0; i < MAX_NUMBER_OF_NODES; i++) {
            if (storage[i][i] != null) {
                myVertice.add((GraphNode) storage[i][i]);
            }
        }
    ...
    public boolean containsVertex(GraphNode vertex) {
        // O(1)
        if (vertex != null) {
            int location = vertex.getId();
            if (storage[location][location] != null && storage[location][location].equals(vertex)) {
                return true;
            }
        }
        return false;
    ...
    public Collection<GraphNode> getNeighbors(GraphNode vertex) {
        ...
        if (!containsVertex(vertex) || vertex == null) {
            return null;
        }
        int location = vertex.getId();
        Collection<GraphNode> myNeighbor = new ArrayList<GraphNode>();
        for (int i = 0; i < MAX_NUMBER_OF_NODES; i++) {
            if (storage[i][location] != null && !myNeighbor.contains(storage[i][i]) && i != location) {
                myNeighbor.add((GraphNode) storage[i][i]);
            }
        }
        int amountOfBlank = 0;
        for (int j = 0; j < MAX_NUMBER_OF_NODES; j++) {
            if (storage[j][location] == null) {
                amountOfBlank++;
            }
        }
        if (amountOfBlank == MAX_NUMBER_OF_NODES - 1) {
            myNeighbor.add((GraphNode) storage[location][location]);
        }
        return myNeighbor;
    }
    ...
    public Collection<GraphEdge> getIncidentEdges(GraphNode vertex) {
        ...
        int location = vertex.getId();
        for (int i = 0; i < MAX_NUMBER_OF_NODES; i++) {
            if (storage[i][location] != null && i != location) {
                myEdge.add((GraphEdge) storage[i][location]);
            }
    ...
    public Pair<GraphNode> getEndpoints(GraphEdge edge) {
        ...
        if (edge != null && getEdges().contains(edge)) {
            ...
            Pair<GraphNode> myPair = new Pair<GraphNode>((GraphNode) storage[location1][location1],
                    (GraphNode) storage[location2][location2]);
            return myPair;
        } else {
            return null;
        }
     ...
     * @param v1 the beginning node to find the edge
     * @param v2 the end node to find the edge
    ...
    public GraphEdge findEdge(GraphNode v1, GraphNode v2) {
        ...
        if (containsVertex(v1) && containsVertex(v2) && v1 != null && v2 != null) {
            int location1 = v1.getId();
            int location2 = v2.getId();
            if (storage[location1][location2] != null) {
                return (GraphEdge) storage[location1][location2];
            }
        }
        return null;
    }
     ...
     * 
     * @param vertex use this vertex that if is incident
     * @param edge use this edge that if is incident
     ...
     * to addEdge(e, new Pair(v1, v2)). If this graph does not contain
     * v1, v2, or both, implementations may choose to either silently add the
     * vertices to the graph or throw an IllegalArgumentException. If this graph
     * assigns edge types to its edges, the edge type of e will be the default for
     * this graph. See Hypergraph.addEdge() for a listing of possible reasons for
     * failure.
     * 
    ...
    public boolean addEdge(GraphEdge e, GraphNode v1, GraphNode v2) {
        ...
        if (!getEdges().contains(e) && e != null && v1 != null && v2 != null) {
            int indexNode1 = v1.getId();
            int indexNode2 = v2.getId();
            if (indexNode1 != indexNode2) {
                storage[indexNode1][indexNode2] = e;
                storage[indexNode2][indexNode1] = e;
                return true;
            }
        }
        return false;
    ...
    public boolean addVertex(GraphNode vertex) {
        ...
        if (vertex == null) {
            ...
            int location = vertex.getId();
            if (vertex != null && !containsVertex(vertex)) {
                storage[location][location] = vertex;
                return true;
            }
        }
        return false;
    ...
    public boolean removeEdge(GraphEdge edge) {
        ...
        for (int i = 0; i < MAX_NUMBER_OF_NODES; i++) {
            for (int j = 0; j < MAX_NUMBER_OF_NODES; j++) {
                if (storage[i][j] != null && storage[i][j].equals(edge) && i != j) {
                    storage[i][j] = null;
                    return true;
                }
            }
    ...
    public boolean removeVertex(GraphNode vertex) {
        ...
        int location = vertex.getId();
        for (int i = 0; i < MAX_NUMBER_OF_NODES; i++) {
            if (storage[i][location] != null) {
                storage[i][location] = null;
                storage[location][i] = null;
            }
        }
        return true;
    }
    ...
    /**
     * Returns a string of the depth first traversal. If curr is not null, the
     * traversal will start at the given vertex. If curr is null, it will pick the
     * lowest ID vertex to start. If restart is set to false, the traversal will
     * only visit vertices reachable from the chosen starting location. If restart
     * is set to true, the depth first traversal will visit the lowest ID vertex not
     * yet visited if it runs out of vertices reachable from the starting location
     * (until all vertices in the graph have been visited).
     * 
     * @param curr    the current vertex being worked on, may be null
     * @param visited the list of vertices which have already been visited by the
     *                algorithm
     * @param restart whether or not to restart if all vertices have not been
     *                reached
    ...
    public String depthFirstTraversal(GraphNode curr, boolean[] visited, boolean restart) {
        ...
        if (restart) {
            ...
            for (GraphNode myNode : getVertices()) {
                if (answer.replace(" ", "").length() == getVertices().size()) {
                    return answer;
                }
                answer += traversal(curr, visited, restart);
            }
    ...
    /**
     * create a private helper method to do traversal.
     * @param curr the current node that begins
     * @param visited make sure if the node is visited or not
     * @param restart make sure if needs to find a new node without edge
     * @return a string of the node
     */
    private String traversal(GraphNode curr, boolean[] visited, boolean restart) {
        String answer = "";
        if (curr == null) {
            for (int i = 0; i < getVertices().size(); i++) {

                if (!visited[i]) {
                    curr = (GraphNode) storage[i][i];
                    break;
                }
            }
        }
        visited[curr.getId()] = true;
        answer = answer + curr.getId() + " ";

        for (GraphNode myNode : getNeighbors(curr)) {

            if (!visited[myNode.getId()]) {
                answer += traversal(myNode, visited, restart);
            }
    ...
    // ********************************************************************************
    /**
     * main method that does a few tests to make sure it works.
     * @param args args
     */
    public static void main(String[] args) {
        // create a set of nodes and edges to test with
        GraphNode[] nodes = { new GraphNode(0), new GraphNode(1), new GraphNode(2), new GraphNode(3), new GraphNode(4),
                new GraphNode(5), new GraphNode(6), new GraphNode(7), new GraphNode(8), new GraphNode(9) };

        GraphEdge[] edges = { new GraphEdge(0), new GraphEdge(1), new GraphEdge(2), new GraphEdge(3), new GraphEdge(4),
                new GraphEdge(5) };
     ...
     * 
     * @param directed_edge the edge that is for directed
     ...
     * undirected edge u if u is incident to both v1 and v2.)
     * @param v1 the first graph node need to be found
     * @param v2 the second graph node need to be found
     ...
     * to addEdge(e, new Pair(v1, v2)). If this graph does not contain
     * v1, v2, or both, implementations may choose to either silently add the
     * vertices to the graph or throw an IllegalArgumentException. If edgeType is
     * not legal for this graph, this method will throw IllegalArgumentException.
     ...
     * </ul>
     * 
     * @param edge edge that need to be used
     * @param vertices vertices that need to be used
     * @return true if the add is successful, and false otherwise
    ...
    /**
     * Adds edge to this graph with type edge_type. Fails under the following
     * circumstances:
     * <ul>
     * <li/>edge is already an element of the graph
     * <li/>either edge or vertices is null
     ...
     * </ul>
     * 
     * @param edge that need to use
     * @param vertices vertices that need 
     * @param edge_type figure which type of the edge
     * @return true if the add is successful, and false otherwise
     * @throws IllegalArgumentException if edge or vertices is null, or if a
     *                                  different vertex set in this graph is
     *                                  already connected by edge, or if vertices
     ...
     * @return return create an instance of this graph type.
     */
